...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setTimeout(() => {
    restoreDraft();
    if (!tutorialShown) {
      showToast('Selamat Datang di v10.0! 👋', 'Klik "Lihat Tutorial" untuk panduan pengisian form', 4000);
    } else {
...
function applySettings(s) {
  ...
  if (hint) {
    if (lm === 'auto') hint.innerHTML = hint.innerHTML.replace(/Pilih.*GPS otomatis/, 'Lokasi akan terdeteksi otomatis via GPS browser');
...
// ===== APPLY META TAGS (Open Graph) — v10.0 =====
function applyMetaTags(s) {
  if (!s) return;
  
  const metaTitle = s.metaTitle || 'DEVIL REIGN v10.0 | Form Pendaftaran Member';
  const metaDescription = s.metaDescription || 'Bergabunglah dengan DEVIL REIGN v10.0 - Komunitas eksklusif Hotel Hideaway. Daftar sekarang dan jadilah bagian dari kekuasaan!';
...
function applyMaintenanceMode(m) {
  ...
  overlay.innerHTML = `
    ...
    <div style="text-align:center;padding:40px;max-width:480px;width:90%">
      ${templateHTML}
      <div style="margin-top:28px;font-size:10px;color:rgba(240,240,245,0.2);letter-spacing:2px">DEVIL REIGN v10.0 © ${new Date().getFullYear()}</div>
    </div>
  `;
...
console.log('%c v10.0 — SEO Enhanced: Full meta tag sync, image validation, SEO score, reset button, live preview ', 'color:#C1121F;font-size:14px;');
console.log('%c Crafted by Hironi × Upgraded by Claude v10.0 ', 'color:#666;font-size:12px;');
