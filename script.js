// ===== DEVIL REIGN — v10.0 =====
// Website by Hironi | Firebase Edition v10.0
// Upgraded: Ad content editor, dynamic promo popup, SEO real-time sync, bug fixes v10.0
...
let bannerImages = [];
let siteSettings = {};
let adContent = null; // v10.0: dynamic ad popup content from Firebase
...
async function loadFromFirebase() {
  try {
    const [adminsRaw, bannersRaw, settings, announcement, popup, maintenance, membersRaw, adContentRaw] = await Promise.all([
      fbGet('admins'),
      fbGet('banners'),
      fbGet('settings'),
      fbGet('announcement'),
    ...
    if (adContentRaw) { adContent = adContentRaw; }
...
function applyMetaTags(s) {
  ...
  const metaUrl = s.metaUrl || s.url || 'https://devilreign.com'; // BUG FIX v10.0: was s.settingUrl (undefined)
  ...
  if (kwEl) kwEl.content = metaKeywords;
...
function startRealtimeSync() {
  setInterval(async () => {
    try {
      ...
      if (settings) { applySettings(settings); applyTheme(settings); applyMetaTags(settings); } // BUG FIX v10.0: added applyMetaTags to realtime sync
      if (announcement) applyAnnouncement(announcement);
      if (maintenance) {
...
// ===== AD POPUP — v10.0 Dynamic =====
function getDefaultAdContent() {
  return {
    badge: 'JASA WEBSITE',
    title: 'Pilih Paket Anda',
    subtitle: 'by HIRONI',
    waLink: 'https://wa.me/6285119654305',
    waLabel: 'Pesan Sekarang',
    packages: [
      { name: 'Biasa',   price: 'Rp 350.000', features: ['Landing Page','3 Section','Mobile Friendly'],          badge: '',           cardClass: 'package-biasa' },
      { name: 'Pro',     price: 'Rp 850.000', features: ['Multi Page','5-7 Halaman','Form Contact','Animasi'],   badge: 'POPULER',    cardClass: 'package-populer' },
      { name: 'Legend',  price: 'Rp 950.000', features: ['Full Website','10+ Halaman','Dashboard Admin','Database'], badge: '',       cardClass: 'package-legend' },
      { name: 'Donatur', price: 'Rp 1.000.000', features: ['Premium Full','Unlimited Fitur','Priority Support','Buat Sultan!'], badge: 'PALING POPULER', cardClass: 'package-donatur' }
    ]
  };
}
...
function renderAdPopup() {
  const inner = document.getElementById('adPopupInner');
  if (!inner) return;
  const d = (adContent && adContent.badge) ? adContent : getDefaultAdContent();
  const pkgs = (d.packages || []).slice(0, 4);

  const pkgsHTML = pkgs.map((p, i) => {
    const badgeHTML = p.badge ? (i === 3
      ? `<div class="package-badge-sultan">${p.badge}</div>`
      : `<div class="package-badge-popular">${p.badge}</div>`) : '';
    const featsHTML = (p.features || []).map(f => `<span class="package-feature">${f}</span>`).join('');
    return `
      <div class="package-card ${p.cardClass || ''}">
        ${badgeHTML}
        <div class="package-header">
          <div class="package-icon">${pkgIcons[i] || pkgIcons[0]}</div>
          <div class="package-info">
            <span class="package-name">${p.name}</span>
            <span class="package-price">${p.price}</span>
          </div>
        </div>
        <div class="package-features">${featsHTML}</div>
      </div>`;
  }).join('');
  ...
  inner.innerHTML = `
    ...
    <div class="ad-popup-header">
      <div class="ad-popup-badge">${d.badge || 'JASA WEBSITE'}</div>
      <h3 class="ad-popup-title">${d.title || 'Pilih Paket Anda'}</h3>
      <p class="ad-popup-subtitle">${d.subtitle || 'by HIRONI'}</p>
    </div>
    <div class="ad-popup-body">
      <div class="pricing-packages">${pkgsHTML}</div>
    </div>
    <div class="ad-popup-footer">
      <a href="${d.waLink || 'https://wa.me/6285119654305'}" target="_blank" class="ad-popup-btn">
        ...
        ${d.waLabel || 'Pesan Sekarang'}
      </a>
      <button class="ad-popup-btn-secondary" onclick="closeAdPopup()">Nanti Saja</button>
    </div>`;
}

function showAdPopup() {
  renderAdPopup(); // v10.0: render dynamic content before showing
  if (adPopupModal) {
...
// Console branding
console.log('%c DEVIL REIGN v10.0 ', 'background:linear-gradient(135deg,#C1121F,#7B0010);color:white;font-size:24px;font-weight:bold;padding:10px 20px;border-radius:10px;');
console.log('%c Firebase Edition v10.0 — Dynamic Ad Editor, SEO Real-Time Sync, Bug Fixes (metaUrl, applyMetaTags sync) ', 'color:#C1121F;font-size:14px;');
