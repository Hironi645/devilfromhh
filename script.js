// ===== DEVIL REIGN — v12.0 =====
// Website by Hironi | Firebase Edition v12.0
// Upgraded: Logo Manager, Meta Tag v12.0, Bug Fix duplikasi meta description
...
let siteLogo = ''; // v12.0: Logo website (dari Firebase settings/logo)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setTimeout(() => {
    restoreDraft();
    if (!tutorialShown) {
      showToast('Selamat Datang di v12.0! 👋', 'Klik "Lihat Tutorial" untuk panduan pengisian form', 4000);
    } else {
...
async function loadFromFirebase() {
  try {
    ...
    // v12.0: Load logo from Firebase settings/logo
    const logoFromSettings = settings && settings.logo ? settings.logo : null;
    if (logoFromSettings) {
      siteLogo = logoFromSettings;
    } else {
      // Fallback: try localStorage (offline mode)
      const localLogo = localStorage.getItem('devilReign_siteLogo');
      if (localLogo) siteLogo = localLogo;
    }
    // Apply logo to #siteLogo element
    const siteLogoEl = document.getElementById('siteLogo');
    if (siteLogoEl && siteLogo) {
      siteLogoEl.src = siteLogo;
      siteLogoEl.onerror = function() {
        // Fallback to default logo if URL broken
        this.onerror = null;
        this.src = 'IMG-20260204-WA0052.jpg';
      };
    }
    ...
    // v12.0: Media popup (gambar/video/gif)
...
// ===== APPLY META TAGS (Open Graph) — v12.0 =====
function applyMetaTags(s) {
  if (!s) return;
  
  const metaTitle = s.metaTitle || 'DEVIL REIGN v12.0 | Form Pendaftaran Member';
  const metaDescription = s.metaDescription || 'Bergabunglah dengan DEVIL REIGN v12.0 - Komunitas eksklusif Hotel Hideaway. Daftar sekarang dan jadilah bagian dari kekuasaan!';
...
function applyMaintenanceMode(m) {
  ...
  overlay.innerHTML = `
    ...
    <div style="text-align:center;padding:40px;max-width:480px;width:90%">
      ${templateHTML}
      <div style="margin-top:28px;font-size:10px;color:rgba(240,240,245,0.2);letter-spacing:2px">DEVIL REIGN v12.0 © ${new Date().getFullYear()}</div>
    </div>
  `;
...
function simulateTyping(targetId, text) {
  ...
  // BUG FIX v12.0: Jangan simulasi jika input sudah ada isi dari user
...
function stopTypingSimulation() {
  isTyping = false;
  if (typingInterval) { clearTimeout(typingInterval); typingInterval = null; }
  // BUG FIX v12.0: Bersihkan placeholder yang mengandung karakter '|' dari simulasi typing
...
function endTutorial() {
  ...
  document.querySelectorAll('.tutorial-target').forEach(el => {
    ...
    el.style.zIndex = '';
    el.style.position = '';
    // BUG FIX v12.0: Restore placeholder yang mungkin diubah saat simulasi typing
    const inp = el.querySelector('.ios-input, .ios-textarea');
...
// Console branding
console.log('%c DEVIL REIGN v12.0 ', 'background:linear-gradient(135deg,#C1121F,#7B0010);color:white;font-size:24px;font-weight:bold;padding:10px 20px;border-radius:10px;');
console.log('%c v12.0 — Logo Manager · Meta Tag SEO Fix · No Duplicate Meta Description ', 'color:#C1121F;font-size:14px;');
console.log('%c Crafted by Hironi × Upgraded by Claude v12.0 ', 'color:#666;font-size:12px;');
