...
async function loadFromFirebase() {
  try {
    ...
    if (mediaPopup && mediaPopup.active && mediaPopup.url) {
      ...
      // Support _forceShow flag dari dashboard admin (untuk reset/tes ulang)
      const mpForceShow = mediaPopup._forceShow &&
        (!sessionStorage.getItem(mpKey + '_force') ||
         mediaPopup._forceShow > parseInt(sessionStorage.getItem(mpKey + '_force') || '0'));
      if (mpForceShow || !sessionStorage.getItem(mpKey)) {
        setTimeout(() => applyMediaPopup(mediaPopup, mpForceShow), (mediaPopup.delay || 2) * 1000);
      }
    }
...
function updateFormProgress() {
  const checks = [
    ...
    /^\d{10,15}$/.test(document.getElementById('nomorWA')?.value.trim()),
...
function applyMediaPopup(data, forced = false) {
  ...
  // BUG FIX v11.0: Strip query params dulu baru cek ekstensi URL
  // Firebase Storage URL bentuknya: .../video.mp4?alt=media&token=xxx
  // endsWith('.mp4') akan SELALU false karena ada query param di belakang!
  const urlBase = (data.url || '').toLowerCase().split('?')[0];
  const mime = (data.mimeType || data.mediaType || data.type || '').toLowerCase();
  const isVideo = mime.startsWith('video') || mime === 'video'
    || data.type === 'video' || data.mediaType === 'video'
    || urlBase.endsWith('.mp4') || urlBase.endsWith('.webm') || urlBase.endsWith('.mov')
    || urlBase.endsWith('.avi') || urlBase.endsWith('.mkv');
  const isGif = urlBase.endsWith('.gif') || urlBase.includes('.gif') || mime === 'gif';
  ...
  const mediaHtml = isVideo
    ? `<video src="${data.url}" autoplay loop muted playsinline controls
       ...
       <div style="display:none;align-items:center;justify-content:center;padding:24px;color:#aaa;font-size:13px;flex-direction:column;gap:8px">
         <div style="font-size:32px">⚠️</div><div>Video gagal dimuat. Coba buka di browser lain.</div>
    ...
    : `<img src="${data.url}" alt="${data.title || 'Media Popup'}"
         ...
         onerror="this.outerHTML='<div style=padding:24px;text-align:center;color:#aaa><div style=font-size:32px>⚠️</div><div style=font-size:13px;margin-top:8px>Gambar gagal dimuat</div><a href=${data.url} target=_blank style=color:#06C5DF;font-size:11px>Buka langsung ↗</a></div>'">`;
  ...
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay && data.closable !== false) overlay.remove();
  });
  ...
  if (forced) {
    // Mode forceShow: tandai timestamp force agar tidak loop, tapi bisa tampil lagi
