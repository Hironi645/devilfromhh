...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setupDraftAutosave();
  setupShareButton();
  setupScrollReveal(); // v12.0: scroll reveal animations
    ...
    if (s.formActive === false) {
      formCard.innerHTML = `
        <div style="padding:40px 24px;text-align:center">
          ...
          <h3 style="font-family:'Bebas Neue',sans-serif;font-size:18px;font-weight:700;margin-bottom:8px">Pendaftaran Ditutup</h3>
...
function applyMaintenanceMode(m) {
  ...
  const templates = {
    classic: `
      ...
      <h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(18px,4vw,26px);font-weight:800;color:${tc.text};line-height:1.2;margin-bottom:14px">${title}</h1>
    ...
    neon: `
      ...
      <h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(18px,4vw,26px);font-weight:900;color:#06C5DF;text-shadow:0 0 20px #06C5DF;letter-spacing:2px;line-height:1.2;margin-bottom:14px">${title}</h1>
  ...
  overlay.style.cssText = `position:fixed;inset:0;z-index:99999;background:${tc.bg};display:flex;align-items:center;justify-content:center;flex-direction:column;font-family:'Outfit',sans-serif;`;
  overlay.innerHTML = `
    <style>
...
function applyDashboardPopup(p) {
  ...
  overlay.innerHTML = `
    ...
    <div style="background:rgba(15,15,20,0.98);border:1px solid ${c.border};border-radius:20px;padding:28px 24px;max-width:360px;width:100%;text-align:center;box-shadow:0 40px 80px rgba(0,0,0,0.7);position:relative;animation:popupIn .35s cubic-bezier(0.16,1,0.3,1)">
      ...
      <div style="font-family:'Bebas Neue',sans-serif;font-size:17px;font-weight:800;color:${c.color};margin-bottom:10px;line-height:1.3">${esc(p.title)}</div>
      ...
      <div style="display:flex;gap:10px;justify-content:center">
        <button id="dpClose2" style="flex:1;padding:11px;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);color:rgba(240,240,245,0.7);border-radius:10px;font-size:13px;cursor:pointer;font-family:'Outfit',sans-serif">Tutup</button>
        ${p.btnUrl ? `<a href="${esc(p.btnUrl)}" target="_blank" id="dpAction" style="flex:1;padding:11px;background:${c.color};color:${p.type==='warn'?'#07070A':'white'};border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;text-decoration:none;display:flex;align-items:center;justify-content:center;font-family:'Outfit',sans-serif">${esc(p.btnLabel||'OK')}</a>` : `<button id="dpAction" style="flex:1;padding:11px;background:${c.color};color:${p.type==='warn'?'#07070A':'white'};border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif">${esc(p.btnLabel||'OK')}</button>`}
      </div>
    </div>`;
...
// ===== ADMIN CARDS =====
function generateAdminCards() { /* Admin selection removed - form now redirects to Grup WA */ }
...
// ===== SELECTED ADMIN =====
function getSelectedAdmin()     { return 'grup'; }
function getSelectedAdminName() { return 'Grup WhatsApp'; }
...
form.addEventListener('submit', async (e) => {
  ...
  if (infoEl) {
    infoEl.innerHTML = `
      ...
      <div class="modal-info-row"><span>💬</span><span>Siap gabung ke Grup WhatsApp</span></div>
    `;
  }
  ...
  if (modalBtn) {
    const handler = () => {
      ...
      // FIX: Kirim pesan WA ke admin pertama, lalu buka link grup
      // Cek apakah grupWALink adalah link invite grup atau nomor WA
      const isGroupInvite = grupWALink.includes('chat.whatsapp.com');
      if (isGroupInvite) {
        // Buka link grup invite — pesan tidak bisa dikirim otomatis ke grup invite
        // Jadi kita buka wa.me ke admin pertama dengan pesan, lalu buka grup
        if (adminList && adminList.length > 0) {
          const firstAdmin = adminList[0];
          const waLink = `https://wa.me/${firstAdmin.nomor}?text=${message}`;
          window.open(waLink, '_blank');
          setTimeout(() => window.open(grupWALink, '_blank'), 1200);
        } else {
          // Fallback: coba buka grup langsung
          window.open(grupWALink, '_blank');
        }
      } else {
        // grupWALink adalah nomor WA langsung
        const waLink = `${grupWALink}?text=${message}`;
        window.open(waLink, '_blank');
      }
      setTimeout(() => showToast('Berhasil! ✅', 'Selamat bergabung di Grup WhatsApp DEVIL REIGN!', 2500), 500);
...
function setupScrollReveal() {
  // Tambahkan kelas reveal ke semua section utama secara otomatis
  ...
  autoRevealSelectors.forEach((sel, i) => {
  ...
  // Form groups reveal stagger
  document.querySelectorAll('.form-group, .input-group').forEach((el, i) => {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        // Jangan un-observe agar tetap visible setelah keluar viewport
      }
    });
  }, {
    threshold: 0.08,
  ...
  // Observe semua reveal elements
  document.querySelectorAll('.reveal, .reveal-left, .reveal-right, .reveal-scale').forEach(el => {
    observer.observe(el);
  });

  // Header langsung visible (sudah di viewport)
  setTimeout(() => {
    document.querySelectorAll('.reveal, .reveal-left, .reveal-right, .reveal-scale').forEach(el => {
