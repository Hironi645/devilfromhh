...
function applyTheme(s) {
  ...
  // Update blob colors (BUG FIX: class aslinya .mesh-blob bukan .blob)
...
function applyMetaTags(s) {
  ...
  const metaImage = s.metaImage || 'https://devilfromhh.vercel.app/IMG-20260204-WA0052.jpg';
...
function startRealtimeSync() {
  setInterval(async () => {
    try {
      ...
      if (settings) { applySettings(settings); applyTheme(settings); applyMetaTags(settings); }
      if (announcement) applyAnnouncement(announcement);
      if (maintenance) {
...
console.log('%c Firebase Edition v9.0 — Dark Mode, Draft, Countdown, Share, Blur Validation, Pagination, Edit Member, Bug Fixes ', 'color:#C1121F;font-size:14px;');
