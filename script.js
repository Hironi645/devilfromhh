...
function renderGrupCards() {
  ...
  if (!container) return;
  const grupNames = {
    grup1: { label: 'Grup Admin 1', desc: 'Tim Utama' },
    grup2: { label: 'Grup Admin 2', desc: 'Tim Pendukung' }
  };
...
form.addEventListener('submit', async (e) => {
  ...
  if (!selectedGrup) {
    const grupErr = document.getElementById('grupError');
    if (grupErr) { grupErr.textContent = 'Wajib pilih grup admin terlebih dahulu'; grupErr.style.display = 'flex'; }
    document.getElementById('grupAdminSection')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    showToast('Pilih Grup ⚠️', 'Pilih grup admin tujuan pengiriman', 3000);
