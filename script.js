...
function applyMediaPopup(data) {
  ...
  // FIX v11.0: Deteksi video dari mediaType ATAU ekstensi URL
  const isVideo = data.mediaType === 'video' ||
    /\.(mp4|webm|ogg|mov)(\?.*)?$/i.test(data.url);

  // FIX v11.0: Deteksi GIF dari mediaType ATAU ekstensi URL (sebelumnya tidak dipakai)
  const isGif = data.mediaType === 'gif' ||
    /\.gif(\?.*)?$/i.test(data.url);

  const mediaHtml = isVideo
    ? `<video src="${data.url}" autoplay loop muted playsinline controls style="width:100%;max-height:70vh;object-fit:contain;display:block;background:#000"></video>`
    : isGif
      ? `<img src="${data.url}" alt="${data.title || 'Media Popup'}" style="width:100%;max-height:70vh;object-fit:contain;display:block">`
      : `<img src="${data.url}" alt="${data.title || 'Media Popup'}" style="width:100%;max-height:70vh;object-fit:contain;display:block">`;
