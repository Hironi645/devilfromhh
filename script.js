...
async function loadFromFirebase() {
  try {
    const [adminsRaw, bannersRaw, settings, announcement, popup, maintenance, membersRaw, adContent] = await Promise.all([
      fbGet('admins'),
      fbGet('banners'),
      fbGet('settings'),
      fbGet('announcement'),
...
function buildDynamicAdPopup(ad) {
  ...
  const STYLE_COLORS = { biasa:'#3D8EFF', populer:'#FFD700', legend:'#8B5CF6', donatur:'#00CC7A' };

  const pkgsHtml = (ad.packages || []).map(p => {
    const color = STYLE_COLORS[p.style] || '#C1121F';
    const feats = (p.features || []).map(f => `<span class="package-feature">${esc(f)}</span>`).join('');
    const badgeHtml = p.badge
      ? `<div class="package-badge-${p.style === 'populer' ? 'popular' : 'sultan'}">${esc(p.badge)}</div>`
      : '';
    return `<div class="package-card package-${esc(p.style || 'biasa')}" style="--pkg-color:${color}">
      ${badgeHtml}
      <div class="package-header">
        <div class="package-icon" style="color:${color}">
  ...
  // Update popup header
  const badgeEl = adPopupModal.querySelector('.ad-popup-badge');
  const titleEl = adPopupModal.querySelector('.ad-popup-title');
  const subtitleEl = adPopupModal.querySelector('.ad-popup-subtitle');
  const pkgContainer = adPopupModal.querySelector('.pricing-packages');
  const btnMain = adPopupModal.querySelector('.ad-popup-btn');
  const btnSec = adPopupModal.querySelector('.ad-popup-btn-secondary');

  if (badgeEl) badgeEl.textContent = ad.badge || 'JASA WEBSITE';
  if (titleEl) titleEl.textContent = ad.title || 'Pilih Paket Anda';
  if (subtitleEl) subtitleEl.textContent = ad.subtitle || 'by HIRONI';
  if (pkgContainer && pkgsHtml) pkgContainer.innerHTML = pkgsHtml;
  if (btnMain) {
    btnMain.textContent = ad.btnText || 'Pesan Sekarang';
    if (ad.btnLink) btnMain.href = ad.btnLink;
  }
  if (btnSec) btnSec.textContent = ad.btnSecondary || 'Nanti Saja';
}
...
// Console branding
console.log('%c DEVIL REIGN v9.0 ', 'background:linear-gradient(135deg,#C1121F,#7B0010);color:white;font-size:24px;font-weight:bold;padding:10px 20px;border-radius:10px;');
console.log('%c v9.0 — SEO Enhanced (keywords, robots, canonical), Dynamic Ad Editor, Multi-Platform Preview ', 'color:#C1121F;font-size:14px;');
console.log('%c Crafted by Hironi × Upgraded by Claude v9.0 ', 'color:#666;font-size:12px;');
